	...
	<body>
		...
		<div id="show-musee" class="content scaffold-show" role="main">
			...
			<ol class="property-list musee">
				...
				<g:if test="${museeInstance?.demandes}">
				<li class="fieldcontain">
					<span id="demandes-label" class="property-label"><g:message code="musee.demandes.label" default="Demandes" /></span>
					
						<g:each in="${museeInstance.demandes}" var="d">
						<span class="property-value" aria-labelledby="demandes-label"><g:link controller="demandeVisite" action="show" id="${d.id}">${d?.encodeAsHTML()}</g:link></span>
						</g:each>
